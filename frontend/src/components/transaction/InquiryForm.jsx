...
const InquiryForm = ({ mode, initialData, onClose, onSave }) => {
        ...
        if (initialData) {
            ...
            if (initialData.isConversion) {
                ...
                const formattedData = {
                    ...
                    followUpTime: initialData.followUpDate ? new Date(initialData.followUpDate).toTimeString().slice(0, 5) : '12:00',
    ...
    const onSubmit = (data) => {
        ...
        if (data.followUpDate) {
            ...
            const combinedDate = new Date(`${data.followUpDate}T${time}`);
                                ...
                                <div>
                                    <div className="flex gap-2">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <TimePicker12Hour 
                                                value={watch('followUpTime')} 
                                                onChange={(val) => setValue('followUpTime', val, { shouldDirty: true })} 
                                            />
                                        </div>
                                    </div>
                                </div>
